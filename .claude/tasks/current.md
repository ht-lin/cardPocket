...
**手动验证**：
- [x] 已登录用户能看到自己的 userName 和 email
- [x] 页面正确调用 GET /api/users/me

**实现**：
- [x] `src/app/(tabs)/profile.tsx`：展示 userName / email，底部操作按钮
- [x] `src/services/userService.ts`：封装 `getMe()`、`updateMe()`、`deleteMe()`
- [x] Tab 栏增加"我的"入口

### ✅ FE-USER-02：修改 userName

- [x] `testUpdateUserNameSuccessfully`：提交新 userName → PATCH /api/users/me → 页面刷新
- [x] `testUpdateUserNameFailsWithDuplicate`：422 重名提示回填到输入框

### ✅ FE-USER-03：修改密码

- [x] `testChangePasswordSuccessfully`：当前密码 + 新密码 → PATCH /api/users/me
- [x] `testChangePasswordFailsWithWrongCurrentPassword`：当前密码错误 → 错误提示

**实现**：Zod schema 验证（新密码强度规则与注册一致），参考 `src/schemas/auth.ts`

### ✅ FE-USER-04：账户注销

- [x] `testDeleteAccountShowsConfirmDialog`：点击注销弹出二次确认
- [x] `testDeleteAccountSuccessfully`：确认后调用 DELETE /api/users/me，清除 token，跳转登录页
