...
6. **增量同步的 deleted 字段**：GET /api/cards?updatedAfter= 的 deleted 数组包含已删除的 Card UUID 和对当前用户撤销的 CardShare 对应的 Card UUID（两种情况都让客户端删本地缓存）。

7. **用户搜索不泄露 email**：搜索结果只返回 id + userName，即使搜索词是 email。

8. **卡片数量上限**：每用户最多 200 张 owned Card。

9. **回收箱（Phase 2）**：删除卡片是软删除（进回收箱），非立即物理删除；保留 30 天后由定时任务物理删除其数据库条目。回收箱**仅含 Owner 自己拥有的卡片**——共享卡被 Owner 删除/过期后，对 Viewer 只从列表移除（走增量同步 `deleted` 数组），不进入任何人的回收箱。

10. **账户级过期策略（Phase 2）**：`User.expiryPolicy` 默认 `KEEP`（卡片过期仅标记、不自动删），可选 `AUTO_TRASH`（过期自动入回收箱）。账户级全局设置，不做单卡片覆盖。
